...
Changing the pins is not persitent, rebooting will change them back to their default setting. Therefore, good practice is to save your configuration as a script. Doing that, reboots would only require you to run the script. Since the pin files are owned by root, **prefixing your command with sudo is required**.
