...
* *robot_workspace/src/PlotJuggler*: Holds the [PlotJuggler](https://github.com/facontidavide/PlotJuggler) package, which is used to visualize data published over ROS topics.
...
* The vicon software creates a client using the [Vicon datastream SDK](https://www.vicon.com/products/software/datastream-sdk) and generates an array of objects over the ROS topic [*object_update*](https://github.com/SjoerdKoop/vicon_control#ros-object_update).
...
* The robot software creates a peer which listens to the [*reference_update*](https://github.com/SjoerdKoop/vicon_control#ros-reference_update) topic and converts the message into an [UDP message](https://github.com/SjoerdKoop/vicon_control#udp-reference) to be send. This peer also listens to the robot and publishes [sensor data](https://github.com/SjoerdKoop/vicon_control#udp-sensor-data) over the [*data_update*](https://github.com/SjoerdKoop/vicon_control#ros-object_update) topic.
...
The software in *robot_workspace*, *vicon_workspace* and *vision_control* is build on the ROS framework, it is required to install and setup ROS to be able to run them. To do so, please follow the instructions described in the [ROS Lunar installation guide](http://wiki.ros.org/lunar/Installation) for your operating system. Another ROS version might still be able to run the software, but is not supported.

### Robot workspace

Currently, *PlotJuggler* requires installation of the ROS type introspection package, which can be installed with:

```
sudo apt-get install ros-lunar-ros-type-introspection
```

Since *PlotJuggler* is third party software, dependencies may change over time. Always check the ROS installation instructions on the [*PlotJuggler* github](https://github.com/facontidavide/PlotJuggler).

### Robot control

To cross compile ARM binaries on Ubuntu 16.04, the appropriate toolchains have to be installed. This is only required when the *robot_control* software is being used. The toolchains for C and C++ can be installed with:
...
The installation script of *robot_control* exports environment variables defining the location of the PRU files, adds these exports to the user's *~/.bashrc* and calls *make*.
...
The firmware of the PRU's is defined in *robot_control/bbb_scripts/start_pru*. **Change *PRU0FW* and *PRU1FW* if needed.**
...
* Change variables in the *Makefile*. Test by running *make* in the project's directory.
...
As shown in the general scheme, communication is an integral part connecting all the subsystems. Communication from the cameras to the Vicon Tracker, and from the Vicon Tracker to the user PC is predefined by Vicon. Therefore, four additional communication instances between nodes are required. Additionally, *[visualization_msgs/Marker](http://docs.ros.org/api/visualization_msgs/html/msg/Marker.html)* messages are send over the topic *marker_update* in the Vicon GUI to generate markers in the rviz screen. Finally, *robot_tools/data_update_array* messages are send over the topic *[data_update](https://github.com/SjoerdKoop/vicon_control#ros-data_update)* from the robot workspace software. The latter is used to visualize data in *PlotJuggler* and might be useful to use when the vision controller should respond to sensor data.
